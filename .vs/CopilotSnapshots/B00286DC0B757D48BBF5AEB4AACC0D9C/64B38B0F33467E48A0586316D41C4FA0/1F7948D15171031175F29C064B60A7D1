...
namespace BlackbodyRadiationExplorer
{
    public partial class MainForm : Form
    {
        ...
        private void AddTemperatureCurve(double temperature)
        {
            ...
            double[] yValues = useLogScale ? 
                radiances.Select(r => Math.Log10(r)).ToArray() : 
                radiances;

            // Create scatter plot with line
            var scatter = formsPlot1.Plot.Add.Scatter(wavelengths, yValues);
            scatter.LineWidth = 2;
        ...
        private void UpdatePlot()
        {
            ...
            foreach (var kvp in temperaturePlots)
            {
                ...
                for (int i = 0; i < wavelengths.Length; i++)
                {
                    ...
                    {
                        double yVal = useLogScale ? Math.Log10(radiances[i]) : radiances[i];
                        
                        if (!double.IsInfinity(yVal) && !double.IsNaN(yVal))
                        {
            ...
            // Apply tight scaling with appropriate padding
            if (minY != double.MaxValue && maxY != double.MinValue)
            {
                ...
                if (useLogScale)
                {
                    // 5% padding for log scale
                    padding = range * 0.05;
                }
                else
                {
                    // 10% padding for linear scale
                    padding = range * 0.10;
                }
        ...
        private void chkLogScale_CheckedChanged(object? sender, EventArgs e)
        {
            ...
            // Redraw all curves with new scale
            UpdateAllCurves();
        }
    }
}
